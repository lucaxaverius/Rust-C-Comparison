#!/bin/bash
MODULE_NAME="Rust_Page_Benchmark"
MODULE_FILE_NAME="rust_page_benchmark"  # Name of your C kernel module
MODULE_SOURCE="rust_page_benchmark.rs"
...
wait_for_test() {
    ...
    while true; do
        if dmesg | tail -n 10 | grep -q "Rust_Page_Benchmark: Module initialization complete"; then
...
for ((run=1; run<=N_RUNS; run++)); do
    ...
    PERF_OUTPUT="$PERF_OUTPUT_DIR/rust-perf-page-$run.data"
