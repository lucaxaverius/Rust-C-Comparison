#!/bin/bash
MODULE_NAME="C-Page-Benchmark"
MODULE_FILE_NAME="page_benchmark"  # Name of your C kernel module
MODULE_SOURCE="page_benchmark.c"
...
I=1         # Initial I value
N_RUNS=250  # Number of iterations 
...
wait_for_test() {
    ...
    while true; do
        if dmesg | tail -n 10    | grep -q "C-Page-Benchmark: Module initialization complete"; then
...
for ((run=1; run<=N_RUNS; run++)); do
    ...
    PERF_OUTPUT="$PERF_OUTPUT_DIR/c-perf-page-$run.data"
    /usr/bin/perf record -e "$PERF_EVENTS" -g -a --freq=1250 --kernel-callchains -o "$PERF_OUTPUT" &
